...
| `APP_ACCESS_CODE` | 앱 접속 코드. 공개 URL 과금 방지용 | 예: `class-2026` |

셋 중 일부만 넣어도 됩니다. 예를 들어 OpenAI 키만 넣으면 OpenAI 분석만 실제 호출되고, Gemini 쪽은 데모 결과가 표시됩니다.

`APP_ACCESS_CODE`를 설정하면 앱 첫 화면에서 접속 코드를 요구합니다. 이 코드를 모르는 사람은 OpenAI/Gemini/알라딘 API를 호출할 수 없습니다. GitHub 저장소가 공개여도 이 값은 Vercel 환경변수에만 저장되므로 GitHub에 노출되지 않습니다.
...
| `GEMINI_TEXT_MODEL` | `gemini-3-flash-preview` | 손금 해석 JSON을 만드는 모델 |
| `GEMINI_IMAGE_MODEL` | `gemini-3.1-flash-image` | 손금 인포그래픽 이미지를 만드는 모델 |
...
1. 배포 주소로 접속합니다.
2. `APP_ACCESS_CODE`를 설정했다면 접속 코드를 입력합니다.
3. 학년, 반, 번호, 이름을 입력합니다.
4. `카메라 켜기`를 누르고 브라우저 카메라 권한을 허용합니다.
5. `사진 찍기`를 누릅니다.
6. OpenAI 또는 Gemini를 선택한 뒤 `분석`을 누릅니다.
7. 결과가 보이면 `PDF 저장/인쇄` 또는 `권장도서`를 누릅니다.

## 공개 URL 과금 방지

이 앱은 Vercel 배포 주소를 아는 사람이 접속할 수 있습니다. AI API 과금을 막으려면 아래 중 최소 1개는 반드시 설정하세요.

1. Vercel 환경변수 `APP_ACCESS_CODE` 설정
2. 수업이 끝나면 Vercel 프로젝트의 환경변수에서 OpenAI/Gemini 키 삭제 또는 프로젝트 일시 중지
3. OpenAI/Gemini에서 사용 한도 또는 결제 한도 설정
4. Vercel `Settings > Deployment Protection`에서 보호 기능 사용

가장 쉬운 방법은 `APP_ACCESS_CODE`입니다. 예를 들어 `class-2026`처럼 정하고, 연수 참여자에게만 알려 주세요.
...
- Gemini 이미지 생성이 실패하면 `GEMINI_IMAGE_MODEL`이 `gemini-3.1-flash-image`인지 확인하세요. 계정 권한 문제가 있으면 `gemini-2.5-flash-image`로 낮춰 테스트하세요.
...
- Gemini 텍스트 분석은 `gemini-3-flash-preview`, 이미지 생성은 `gemini-3.1-flash-image`를 기본값으로 사용합니다. 접근 권한 문제가 있으면 이미지 모델은 `gemini-2.5-flash-image`, 텍스트 모델은 `gemini-2.5-flash`로 낮춰 테스트할 수 있습니다.
